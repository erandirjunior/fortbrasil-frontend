...
import Account from 'builder/forms/Account'
import { required, email } from 'vuelidate/lib/validators'
import alert from 'src/app/infrastructure/components/alert/alert'
import { get, put } from 'src/app/infrastructure/components/request/request'
  ...
  methods: {
    ...
    update () {
      ...
      put(`users/${this.form.id}`, this.form)
        .then(() => {
          alert('Sucesso ao atualizar dados!', 'positive', 'thumb_up')
          this.$router.push('/')
        })
        .catch(error => {
